...
class Test:
    ...
    def test(self):
        ...
        # 进行正则表达式判断
        result1 = self.network_obj.check_networkcard(network_card=self.node2["sub_bond_name"][0],ssh_conn=self.obj_list[1])
        # 进行正则表达式判断
        result1 = self.network_obj.check_networkcard(network_card=self.node3["sub_bond_name"][0],ssh_conn=self.obj_list[2])
        # 进行正则表达式判断
