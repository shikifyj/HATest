...
class Test:
    ...
    def test(self):
        ...
        self.network_obj.close_heartbeat(network_card=self.node1["sub_bond_name"][0],ssh_conn=self.obj_list[0])
        ...
        self.network_obj.recovery_heartbeat(network_card=self.node1["sub_bond_name"][0],ssh_conn=self.obj_list[0])
        result1 = self.network_obj.check_networkcard(network_card=self.node1["sub_bond_name"][0],ssh_conn=self.obj_list[0])
        # 进行正则表达式判断
