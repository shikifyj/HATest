...
class Test:
    ...
    def test(self):
        ...
        """修改资源所在节点的RA"""
        self.pacemaker_obj.restart_resources(ssh_conn=self.obj_list[0])

        result1 = self.network_obj.check_networkcard(network_card=self.node1["sub_bond_name"][0],ssh_conn=self.obj_list[0])
        # 进行正则表达式判断

        """修改资源所在节点的RA，将加入的sleep语句删除"""
        """刷新资源"""
        self.pacemaker_obj.move_resources(node=self.node2,ssh_conn=self.obj_list[2])
